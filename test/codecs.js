import assert from 'assert';
import {
  BooleanCodec,
  Float64Codec,
  FloatPrecisionCodec,
  Int16Codec,
  Int32Codec,
  Int8Codec,
  NullCodec,
  StringCodec,
  StringLongCodec,
  UInt16Codec,
  UInt32Codec,
  UInt8Codec,
  UIntLongCodec,
} from '../src/server/index.js';

describe('UInt8Codec', () => {
  const codec = new UInt8Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode 255', () => {
    codec.encode(buffer, 1, 255);
    assert.equal(codec.decode(buffer, 1), 255);
  });
});

describe('UInt16Codec', () => {
  const codec = new UInt16Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode 65535', () => {
    codec.encode(buffer, 1, 65535);
    assert.equal(codec.decode(buffer, 1), 65535);
  });
});

describe('UInt32Codec', () => {
  const codec = new UInt32Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode 131071', () => {
    codec.encode(buffer, 1, 131071);
    assert.equal(codec.decode(buffer, 1), 131071);
  });
});

describe('UIntLongCodec', () => {
  const codec = new UIntLongCodec(5);
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 623804400001', () => {
    codec.encode(buffer, 1, 623804400001);
    assert.equal(codec.decode(buffer, 1), 623804400001);
  });
});

describe('BooleanCodec', () => {
  const codec = new BooleanCodec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode true', () => {
    codec.encode(buffer, 1, true);
    assert.equal(codec.decode(buffer, 1), true);
  });

  it('Should encode/decode false', () => {
    codec.encode(buffer, 1, false);
    assert.equal(codec.decode(buffer, 1), false);
  });
});

describe('StringCodec', () => {
  const codec = new StringCodec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength('Hell0 wœrld$ 🌝 !'));

  it('Should encode/decode "Hell0 wœrld$ 🌝 !"', () => {
    codec.encode(buffer, 1, 'Hell0 wœrld$ 🌝 !');
    assert.equal(codec.decode(buffer, 1), 'Hell0 wœrld$ 🌝 !');
  });
});

describe('StringLongCodec', () => {
  const codec = new StringLongCodec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength('data:image/jpeg;base64,'));

  it('Should encode/decode a long string', () => {
    codec.encode(buffer, 1, 'data:image/jpeg;base64,');
    assert.equal(codec.decode(buffer, 1), 'data:image/jpeg;base64,');
  });
});

describe('Int8Codec', () => {
  const codec = new Int8Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode -128', () => {
    codec.encode(buffer, 1, -128);
    assert.equal(codec.decode(buffer, 1), -128);
  });

  it('Should encode/decode 127', () => {
    codec.encode(buffer, 1, 127);
    assert.equal(codec.decode(buffer, 1), 127);
  });
});

describe('Int16Codec', () => {
  const codec = new Int16Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode -32768', () => {
    codec.encode(buffer, 1, -32768);
    assert.equal(codec.decode(buffer, 1), -32768);
  });

  it('Should encode/decode 32767', () => {
    codec.encode(buffer, 1, 32767);
    assert.equal(codec.decode(buffer, 1), 32767);
  });
});

describe('Int32Codec', () => {
  const codec = new Int32Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode -2147483648', () => {
    codec.encode(buffer, 1, -2147483648);
    assert.equal(codec.decode(buffer, 1), -2147483648);
  });

  it('Should encode/decode 2147483647', () => {
    codec.encode(buffer, 1, 2147483647);
    assert.equal(codec.decode(buffer, 1), 2147483647);
  });
});

describe('Float64Codec', () => {
  const codec = new Float64Codec();
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode 1.123', () => {
    codec.encode(buffer, 1, 1.123);
    assert.equal(codec.decode(buffer, 1), 1.123);
  });

  it('Should encode/decode -12456789.123456789087654321012345678901', () => {
    codec.encode(buffer, 1, -12456789.123456789087654321012345678901);
    assert.equal(codec.decode(buffer, 1), -12456789.123456789087654321012345678901);
  });
});

describe('FloatPrecisionCodec(Int8Codec, 2)', () => {
  const codec = new FloatPrecisionCodec(Int8Codec, 2);
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode 0.99', () => {
    codec.encode(buffer, 1, 0.99);
    assert.equal(codec.decode(buffer, 1), 0.99);
  });

  it('Should encode/decode -0.65', () => {
    codec.encode(buffer, 1, -0.65);
    assert.equal(codec.decode(buffer, 1), -0.65);
  });

  it('Should encode/decode 0.1', () => {
    codec.encode(buffer, 1, 0.1);
    assert.equal(codec.decode(buffer, 1), 0.1);
  });

  it('Should encode/decode 1.27', () => {
    codec.encode(buffer, 1, 1.27);
    assert.equal(codec.decode(buffer, 1), 1.27);
  });

  it('Should encode/decode -1.28', () => {
    codec.encode(buffer, 1, -1.28);
    assert.equal(codec.decode(buffer, 1), -1.28);
  });
});

describe('FloatPrecisionCodec(UInt32Codec, 3)', () => {
  const codec = new FloatPrecisionCodec(UInt32Codec, 3);
  const buffer = new ArrayBuffer(1 + codec.getByteLength());

  it('Should encode/decode 0', () => {
    codec.encode(buffer, 1, 0);
    assert.equal(codec.decode(buffer, 1), 0);
  });

  it('Should encode/decode 4294967.295', () => {
    codec.encode(buffer, 1, 4294967.295);
    assert.equal(codec.decode(buffer, 1), 4294967.295);
  });

  it('Should encode/decode 123456.999', () => {
    codec.encode(buffer, 1, 123456.999);
    assert.equal(codec.decode(buffer, 1), 123456.999);
  });

  it('Should encode/decode 5.2', () => {
    codec.encode(buffer, 1, 5.2);
    assert.equal(codec.decode(buffer, 1), 5.2);
  });
});
